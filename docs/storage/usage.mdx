...
FlutterFire also supports uploading lowerlevel typed data in the form of a [`ByteBuffer`](https://api.dart.dev/stable/2.9.2/dart-typed_data/ByteBuffer-class.html)
for those cases where uploading a string or `File` is not practical. In this case, call the [`put`](!firebase_storage.Reference.put)
method with your buffer:
...
ByteBuffer buffer = Uint8List.fromList(encoded).buffer;
