...
To create a new Firestore instance, call the [`instance`](!cloud_firestore.Firestore.instance) getter on [`FirebaseFirestore`](!cloud_firestore.FirebaseFirestore):
...
await FirebaseFirestore.instance.settings = Settings(host: "10.0.2.2", sslEnabled: false);
```
