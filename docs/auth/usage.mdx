...
`idTokenChanges()` method instead. If you want to listen for any user changes, you can subscribe to the `userChanges()` method.
...
will create a new unique user which will be persisted across app restarts/page reloads. If the user signs-out and re-authenticates
...
As mentioned above, some operations such as deleting the user or updating their email address require the user to have recently
