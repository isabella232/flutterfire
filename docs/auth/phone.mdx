...
- **Web Platform**: [~~`signInWithPhoneNumber`~~ `Currently unavailable`](!firebase_auth.FirebaseAuth.signInWithPhoneNumber)
...
resolved an SMS message within a certain time frame. Once the time frame has passed, the device will no longer attempt to resolve
